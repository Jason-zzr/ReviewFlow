...
| Local data | Versioned SQLite business state, application-managed media, portable import/export and redacted diagnostics | Desktop `PRAGMA user_version`, publisher `schema_meta`, managed media plus workspace bundle round-trip/integrity tests, and diagnostic allowlist | Locally verified |
...
| Windows distribution | No system Python required; embedded Sidecar/uploader; clean installation path works without orphan runtimes | NSIS temporary clean install, minimal-`PATH` startup, forced-parent-exit lifecycle check, Sidecar and uploader smoke tests | Locally verified on current Windows host |
...
- Domain: 16 tests passed.
- Electron main process: 8 tests passed, including portable workspace media round-trip, legacy import compatibility, and linked-directory escape rejection.
- Publisher: 49 tests passed; one non-blocking Starlette/httpx deprecation warning.
- Installer: `apps/desktop/release/ReviewFlow-0.1.0-x64-setup.exe`.
- Installer SHA-256: `b191f7269473f0498a7235d40b505b457ed5fdd58a81afa22b3f92fc6d692f60`.
